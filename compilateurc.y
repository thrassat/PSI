...
Exp : Exp tPLUS Exp { int ad1 = get_addr_fi($1); // c'est effectivement from index car Value est un index  (de son tmp)
	...
	| Exp tMUL Exp { int ad1 = get_addr_fi($1); 
					int ad2 = get_addr_fi($3) ; 
					insert2("LOAD",0,ad1) ; //printf("LOAD 0, %d\n",ad1) ; 
					insert2("LOAD",1,ad2) ; //printf("LOAD 1, %d\n",ad2) ;
...
If : tIF tPARO Comparaison tPARF XIF
	...
	| tIF tPARO Comparaison tPARF XIF
			...
			tELSE 
				...
				$7 = get_last_ti() - 1 ;} 	    //Se repositionne juste av le Else
			Body 
...
While : tWHILE 
		...
		tPARO Comparaison tPARF
			...
			Body 
				{insert2("JMP",$1,1);
				 set_ti($3,"a",get_last_ti()); }  ; // a ou b a voir
